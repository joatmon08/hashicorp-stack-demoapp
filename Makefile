...
configure-vault:
	...
	cd vault && terraform init && terraform apply cd vault && terraform init && terraform apply -var postgres_hostname=$(shell cd infrastructure && terraform output -raw product_database_address) -var postgres_password=${PGPASSWORD}
