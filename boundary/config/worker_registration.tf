...
data "vault_kv_secret_v2" "boundary_worker_token_database" {
  depends_on = [data.vault_kv_secrets_list_v2.boundary_worker_tokens]
  mount      = local.boundary_worker_mount
  name       = split(".", local.boundary_worker_database_dns).0
}

resource "boundary_worker" "database" {
  scope_id                    = "global"
  name                        = data.vault_kv_secret_v2.boundary_worker_token_database.name
  description                 = "self-managed worker ${data.vault_kv_secret_v2.boundary_worker_token_database.name} for databases (general)"
  worker_generated_auth_token = data.vault_kv_secret_v2.boundary_worker_token_database.data.token
}